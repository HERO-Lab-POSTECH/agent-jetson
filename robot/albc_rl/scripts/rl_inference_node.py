...
class RLInferenceNode(object):
    def __init__(self):
        ...
        # --- ARM PROTECTION (2026-08-25, calibrated on the arm2 fracture run) --
        # Sustained current, NOT peak: breakaway from rest legitimately draws
        # 1.1-1.3 A, and the arm1 break was 1.0 A sustained. <= 0 on either knob
        # disables that check. joint2 has NO hard angle window on purpose
        # (rescinded 2026-08-26) -- singularity avoidance is a trained cost.
        # Why each number is what it is, and what is decision vs measurement:
        #   docs/adr/002-arm-guard-current-cap.md
