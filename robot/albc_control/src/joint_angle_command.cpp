// joint_angle_command.cpp — Dynamixel arm driver (RL deployment build)
// ------------------------------------------------------------------------------
// Publishes /albc/joint_states so the RL inference node reads the arm state
// without opening the serial port, which has a single owner: this node.
//
// TWO CONTRACT CHOICES, decided against the sim (observations.py):
//   1. POSITION published is the MEASURED encoder value (unwrapped, cumulative),
//      never the commanded absolute_angle -- the sim obs is true state.
//   2. VELOCITY is differentiated HERE, at the driver's real rate, because the
//      RL node runs 5x faster and re-differentiating would see a staircase.
//
// Guard behaviour (deliberate, do NOT soften): ~joint1_abort_rad ABORTS the run
// and holds the last goal with torque ON; the constraint meter on
// /albc/joint_guard COUNTS and never clamps.
//
// Every change this build made to the board original, with its evidence:
//   docs/adr/004-driver-rl-deployment-patches.md
// ------------------------------------------------------------------------------
#include "ros/ros.h"
