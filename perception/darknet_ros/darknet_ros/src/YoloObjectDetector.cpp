...
void YoloObjectDetector::init()
{
  ...
  boundingBoxPublisher_ = nodeHandle_.advertise<darknet_ros_msgs::BoundingBox>("/darknet_ros/bounding_box",1);
...
imagePublisher_ = imageTransport_.advertise("/image_converter/dark", 1);
...
{
set_ipl(buff_[(buffIndex_ + 1)%3], ipl_);
  cv::Mat cvImage = cv::cvarrToMat(ipl_);

  //cv::imwrite("/home/nvidia/dddd.jpg", cvImage);
  //imshow("fff",cvImage);
  /*
...
sensor_msgs::ImagePtr msg_output = cv_bridge::CvImage(std_msgs::Header(), "bgr8", cvImage).toImageMsg();
  //detectionImagePublisher_.publish(msg_output);
imagePublisher_.publish(msg_output);
    ...
    for (int i = 0; i < numClasses_; i++) {
      ...
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          boundingBoxPublisher_.publish(boundingBox);
