...
**내부 설계**:
메시지 스키마는 순수 평탄 구조(nested 타입 없음), little-endian ROS 표준. 각 .msg 파일은 catkin message_generation을 트리거하여 devel/include/hero_msgs/*.h (C++), devel/lib/python*/dist-packages/hero_msgs/*.py (Python), 그리고 firmware rosserial 스텁을 생성한다. DEPRECATED 필드 (e.g., cont_para의 일부 토픽)는 문서화되어 있으며, 필드 삭제는 firmware re-flash가 필요하므로 지연되었다.
...
**내부 설계**:
v2.0.0 단일-노드 재설계 (이전 3-노드 파이프라인 병합). 제어 흐름: key_teleop.py → Int8 → agent.cpp key_input_callback (allow-list KEYMAP 조회, debounce 게이트) → fw_char 또는 번역된 action (TeleopController 인프로세스, 주제 왕복 없음). 100 Hz 메인 루프: 타겟 dirty 플래그 설정 시 /hero_agent/dvl 발행, csv_rate_hz에서 CSV 쓰기 (50 Hz 기본값), 모든 반복에서 모니터 렌더링. AsyncSpinner(2)는 4개 구독자 (key_input, state, sensors, albc_status) 처리. 동기화: atomic<bool> g_target_dirty (lock-free), std::mutex for CSV/ALBC 캐시, per-char debounce via ros::Time[256] 배열. StateMonitor는 imu_yaw_offset 파람에서 45° IMU 프레임 회전 적용. RosbagRecorder는 rosbag record를 fork/exec, 5개 토픽 기록, SIGTERM→poll(30×100ms)→SIGKILL으로 정리, .active 이름 변경. CSV 로거는 17-col 고정 헤더로 각 쓰기마다 flush. KEYMAP은 24개 항목 allow-list SSOT; 미등록 키는 drop.
...
- /albc_status 상태 브로드캐스트 (11-field 계약)
...
| `include/albc_control/status_publisher.h` | /albc_status 발행 (11-field 계약) | 83 |
...
- /albc_status: [0-3] attitude deg (RAD2DEG), [4-7] EE x/y m, [8-10] angular vel rad/s
...
- /albc_status 필드 순서 취약성: 3곳에 동결됨 (코멘트만, 스키마 검증 없음)
...
| 파일 | 역할 | 라인 |
|:---|:---|:---:|
| `scripts/rl_inference_node.py` | 메인 노드: 50 Hz 타이머, 상태 보유 | 375 |
| `scripts/build_proprio.py` | 센서 surface: 20D proprioception 어셈블러 | 230 |
| `scripts/test_build_proprio.py` | 테스트: 33개 테스트 (PASS) | 345 |
| `numpy_port/np_policy.py` | 정책 런타임: obs 어셈블리, history buffer | 172 |
...
| `~use_board_rates` | param | bool (true ⇒ obs[6:9] from /albc_status[8:11]) |
| `~imu_yaw_offset_deg` | param | float (default 45.0, live-tunable) |
...
**정책 런타임** (NumpyStudentPolicy): .npz 가중치 (학생 + 교사)를 TCN 또는 GRU encoder + TeacherActor로 로드. 각 act() 호출에서 69D obs = [proprio_20 + jb_hist_30 + act_hist_16 + integral_3] 어셈블. History buffer는 stride-3 링 (3-step window = 18D/step). Integral은 3D leaky gated on [roll_att_err, pitch_att_err, yaw_rate_err] with leak=0.99, sigma=0.10, clamp=±2.0. 관절 PD 목표 누적 += DELTA_SCALE*action[:2]. 클립된 액션 ∈[-1, 1] 반환.

**Encoder** (TCN 또는 GRU): TCN: channel_transform + 3×Conv1d + head. GRU: 1-layer GRU + same head.

**Actor** (TeacherActor): 69D obs 정규화 (EmpiricalNormalization) + 9D latent 연결 → 78D → 4-layer MLP (ELU + softsign).
...
- **정책 관측 69D**: proprio_20 + jb_hist_30 + act_hist_16 + integral_3
...
**69D 정책 관측** (NumpyStudentPolicy.act input):
```
proprio_20 (as above) +
...
2. hero_agent, albc_control (add_dependencies on message generation)
...
├── hero_msgs/
│   ├── msg/
│   │   ├── hero_agent_sensor.msg       (8 fields: ROLL, PITCH, YAW, DEPTH, GYRO_*)
│   │   ├── hero_agent_state.msg        (7 fields)
│   │   ├── hero_agent_dvl.msg          (3 fields: TARGET_X/Y/Z)
│   │   └── ... (6 more .msg files)
│   ├── NAMING_CONVENTION.md            (178 lines, REP-103/135/145 SSOT)
│   ├── CMakeLists.txt                  (message_generation 선언)
│   └── package.xml
│
├── hero_agent/
│   ├── src/
│   │   └── agent.cpp                   (297 LOC, 메인 노드)
│   ├── include/hero_agent/
│   │   ├── keymap.h                    (59 LOC, 24-key allow-list)
│   │   ├── key_translator.h            (19 LOC, pure fn)
│   │   ├── teleop_controller.h         (42 LOC, stateful accumulator)
│   │   ├── state_monitor.h             (161 LOC, callback + cache + IMU rotation)
│   │   ├── csv_logger.h                (93 LOC, file I/O)
│   │   ├── rosbag_recorder.h           (84 LOC, subprocess mgmt)
│   │   └── topics.h                    (19 LOC, topic SSOT)
│   ├── scripts/
│   │   └── key_teleop.py               (85 LOC, tty reader)
│   ├── config/
│   │   └── agent.yaml                  (22 LOC, params)
│   ├── launch/
│   │   └── agent_launch.launch         (36 LOC)
│   ├── CMakeLists.txt
│   ├── package.xml                     (manifest v1.0.0, CHANGELOG v2.1.0)
│   └── CHANGELOG.md
│
├── albc_control/
│   ├── src/
│   │   ├── albc_controller.cpp         (345 LOC, 50 Hz loop)
│   │   └── joint_angle_command.cpp     (354 LOC, Dynamixel 10 Hz driver)
│   ├── include/albc_control/
│   │   ├── attitude_controller.h       (250 LOC, 4-mode feedback)
│   │   ├── inverse_kinematics.h        (95 LOC, DLS)
│   │   ├── mode_manager.h              (250 LOC, FSM + termios)
│   │   ├── imu_processor.h             (53 LOC, sensor callback)
│   │   ├── status_publisher.h          (83 LOC, 11-field contract)
│   │   ├── control_law.h               (114 LOC, oracle)
│   │   ├── feedback_filters.h          (80 LOC, oracle)
│   │   ├── dls_ik.h                    (92 LOC, oracle)
│   │   ├── imu_rotation.h              (46 LOC, oracle)
│   │   ├── albc_kinematics.h           (59 LOC, pure kinematics)
│   │   ├── dashboard.h                 (76 LOC, UI)
│   │   ├── joint_current_monitor.h     (42 LOC, cache)
│   │   └── dynamixel_config.h          (81 LOC, constants)
│   ├── config/
│   │   └── albc_controller.yaml        (50 LOC)
│   ├── cfg/
│   │   └── ALBCController.cfg          (54 LOC, dynamic_reconfigure)
│   ├── launch/
│   │   └── albc.launch                 (13 LOC)
│   ├── CMakeLists.txt
│   ├── package.xml                     (manifest v2.0.0)
│   └── CHANGELOG.md
│
├── albc_rl/
│   ├── scripts/
│   │   ├── rl_inference_node.py        (375 LOC, 50 Hz timer loop)
│   │   ├── build_proprio.py            (230 LOC, 20D sensor surface)
│   │   └── test_build_proprio.py       (345 LOC, 33 tests PASS)
│   ├── numpy_port/
...
│   │   │   └── ... (no golden_gru.npz)
│   │   ├── weights_teacher.npz         (배포 가중치)
│   │   ├── weights_tcn.npz             (배포 가중치)
│   │   └── ... (no weights_gru.npz)
│   ├── cfg/
...
│   └── rosserial_msgs/
│
└── firmware/                          (분석 범위 외 — robot/와의 경계만 참조)
    ├── agent/
    │   ├── agent.ino                   (519 LOC, 7-모듈 분리 후 메인)
    │   ├── ahrs.cpp/h                  (137/36 LOC, MicroStrain AHRS 파서)
    │   ├── dvl_position.cpp/h          (167/52 LOC)
    │   ├── io.cpp/h                    (57/12 LOC)
    │   ├── pid.cpp/h                   (84/50 LOC)
    │   ├── thrusters.cpp/h             (24/14 LOC, ESC 혼합기)
    │   └── config.h                    (46 LOC)
    ├── libraries/
    │   └── BlueRobotics_MS5837/        (depth sensor I2C 벤더 라이브러리)
    ├── regen_ros_lib.sh                (rosserial_arduino make_libraries.py)
    ├── BUILD_AND_FLASH.md
    └── README.md
```

> **범위 주석**: `drivers/`·`firmware/`는 본 문서의 분석 대상(robot/ 4패키지)에 포함되지 않는다. 위 트리는 robot/ 패키지와의 *인터페이스 경계*(rosserial 브릿지, Dynamixel SDK, IMU/depth 펌웨어)를 식별하기 위한 참조이며, LOC만 실측했을 뿐 내부 설계는 감사하지 않았다. 펌웨어 빌드·flash 절차는 `firmware/BUILD_AND_FLASH.md`를 SSOT로 한다.
