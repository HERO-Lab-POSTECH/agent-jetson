...
int main()
{
    ...
    {
        double i = integralStep(0.0, 2.0, TH, IMAX);
        expect_near(i, 2.0, "integral: accumulate (0 + 2)");          // hand: 0+2
    }
    // Repeated accumulation: feed result back in.
    {
        double i = 0.0;
        i = integralStep(i, 1.5, TH, IMAX); // 1.5
        i = integralStep(i, 1.5, TH, IMAX); // 3.0
        i = integralStep(i, 1.5, TH, IMAX); // 4.5
        expect_near(i, 4.5, "integral: 3x accumulate (1.5*3)");       // hand: 4.5
    }
    // Negative error accumulates negatively.
    {
        double i = integralStep(10.0, -4.0, TH, IMAX);
        expect_near(i, 6.0, "integral: negative error (10 + -4)");    // hand: 6.0
    }

    // ---------------------------------------------------------------------
    // 2. INTEGRAL — FREEZE at setpoint (|error| < threshold): integral held.
    // ---------------------------------------------------------------------
    // error 0 (< threshold): integral unchanged. 3e169b7 moved this gate off the
    // measured angle onto the error -- an angle of 999 no longer unfreezes it.
    {
        double i = integralStep(7.0, 0.0, TH, IMAX);
        expect_near(i, 7.0, "integral: frozen at error=0 (held)");    // hand: held -> 7.0
    }
    // angle just BELOW threshold: still frozen.
    {
        double below = TH - 1e-6;
        double i = integralStep(3.0, below, TH, IMAX);
        expect_near(i, 3.0, "integral: frozen just below threshold"); // hand: held -> 3.0
    }
    // angle EXACTLY at threshold: accumulates (>= is inclusive).
    {
        double i = integralStep(3.0, TH, TH, IMAX);
        expect_near(i, 3.0 + TH, "integral: accumulates AT threshold (>=)");// hand: 3+TH
    }
    // negative angle below threshold magnitude: frozen (abs() used).
    {
        double i = integralStep(2.0, -(TH - 1e-6), TH, IMAX);
        expect_near(i, 2.0, "integral: frozen at small negative error");// hand: held
    }
    ...
    {
        double i = integralStep(99.0, 50.0, TH, IMAX);
